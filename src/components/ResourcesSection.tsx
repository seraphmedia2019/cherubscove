...
const iconMap = {
  audio: { icon: Headphones, bg: 'bg-orange-soft', color: 'text-primary' },
  video: { icon: Video, bg: 'bg-gold/10', color: 'text-gold' },
  pdf: { icon: FileText, bg: 'bg-bg-subtle', color: 'text-muted-foreground' },
};
...
export default function ResourcesSection() {
  ...
  return (
    <section id="resources" className="py-24 bg-bg-alt border-t border-border" ref={ref}>
      <div className="container">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-10 gap-6 reveal">
          <div>
            <div className="eyebrow">Ministry Resources</div>
            <h2 className="section-title" style={{ fontSize: 'clamp(26px,3.5vw,40px)', margin: '0.5rem 0 0' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border rounded overflow-hidden reveal">
          {filtered.map((res, i) => {
            ...
            return (
              <div
                key={i}
                className="bg-card p-8 flex flex-col gap-3.5 transition-colors duration-250 hover:bg-bg-subtle cursor-pointer card-lift"
              >
