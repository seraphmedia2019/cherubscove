...
const contactItems = [
  ...
  { icon: <Phone size={18} />, label: 'Phone / WhatsApp', value: '+234 000 000 0000' },
  { icon: <MapPin size={18} />, label: 'Based In', value: 'Nigeria' },
];
              ...
              <div className="bg-card border border-border rounded p-8">
                ...
                <div className="flex gap-2.5 flex-wrap mt-8 pt-7 border-t border-border">
                  {socialLinks.map((link, i) => (
                    <a key={i} href={link.href} aria-label={link.label} className="w-[38px] h-[38px] rounded-lg border-[1.5px] border-border flex items-center justify-center text-muted-foreground transition-all duration-250 hover:border-primary hover:bg-orange-soft hover:text-primary">
                      {link.icon}
                    </a>
                  ))}
                </div>
